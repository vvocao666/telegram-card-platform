...
- 群内 `/统计` 按用户汇总当前群北京时间当天的图片数和已识别 PUBG/PSN 卡密数；`/统计` 统计 00:00 至命令发送时间，`/统计12:01` 统计 12:01 至命令发送时间，`/统计12:00-18:00` 统计指定区间。仅排除全局机器人 owner，其他用户正常统计；重复卡密在所选区间内保留第一次出现并只计一次，重复图片仍计入发送图片数。
- 稳定去重、疑似重复提醒。
- 记账、汇率、费率、日切、价格查询和账单。
- owner 私聊群组广播。
- 当前群通知所有人。
