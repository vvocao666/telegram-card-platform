...
- 已确认的精确卡段规则可以应用，但必须通过 Validator。
...
当前 `/learn_cards` 和“学习卡密”流程用于人工真值学习。讨论中的 `/训练开始`、发送错误图片、提交正确卡、`/训练结束` 简化流程尚未实现，不能当成现有功能。

未来如实现简化训练，只能采用旧模型后的安全纠错记忆：一次样本只记完整错误卡到正确卡；字符级泛化必须满足同字体、同位置、同上下文和重复次数阈值。不得在线直接修改生产模型权重。
