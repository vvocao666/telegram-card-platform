...
- 状态面板和 OCR 调试命令。
- `/上课`、`/下课` 群状态控制。
- 群内 `/del` 由全局机器人 owner 使用，机器人须为管理员并具备当前群删除权限。
  机器人不是管理员或没有删除权限时静默跳过，不发送权限提示。
  从命令消息编号向前分批清理，48 小时限制由 Telegram 删除接口执行；
  已删除、过期和不可删除的系统消息会跳过，不影响账本、卡密缓存与审计数据。
  清理开始后的新消息保留；一次只运行一个当前群清理任务，限流时按要求等待。
  开始、正常完成及重复执行时静默，不发送清理进度或完成提醒。
  清理期间再次发送 `/del` 会扩展清理范围，下一批优先处理新命令之前的消息，
  不会启动重复任务，也不会跳过 Telegram 要求的限流等待。
  任务失败明确提示中断，不报告全部完成。同一进程内跳过已经完成清理的编号范围；
  首次运行或重启后需扫描历史编号，历史量较大时清理耗时增加。
- owner 私聊 `/del` 打开与广播相同的群多选列表，支持翻页、取消和确认清理。
  群列表仅显示实时确认机器人具有删除权限的群，权限查询失败的群不显示。
  私聊立即显示加载状态，最多同时查询 5 个群，单群最多等待 8 秒、整次最多
  等待 20 秒；仅展示已核实的群，查询受限时按 Telegram 要求暂停后续查询。
  普通 Telegram 请求池允许 8 条连接，支持权限并发查询并为消息回复留出连接。
  仅清理明确选中的群，执行前检查机器人删除权限，进度和结果只在私聊显示。
  私聊清理以点击确认时各群已记录的最新消息编号为上界，之后的新消息保留。
  只持久化各群最新消息编号，不保存聊天内容；记录位于账本同目录的
  `*.message-positions.sqlite3`，独立于账本，重启后保留。
  新版本尚未收到消息的群需要先在群内产生一条消息；没有记录的群明确跳过。
  旧菜单、已确认或已取消的菜单不能再次执行，不影响广播的选择状态。
- TRC20 地址防篡改/校验模块。
- systemd、一键部署、备份和恢复。
...
- 群级汇率、费率和日切。
- 账单汇率和费率同一行，以 ` | ` 分隔。固定汇率不补小数位；
  通过“设置实时汇率”成功获取后显示实际数值并保留两位小数。
  账单金额和费率去掉末尾多余的零，整数不显示 `.00`；计算精度和账目不变。
  汇率来源标记在重启后保留；旧设置未记录来源，须重新设置才能标记为实时汇率。
- 汇率/费率快照。
- 手续费、应下发人民币和 U。
- 今日、昨日和完整账单。
- 账单按钮可在群级“简洁模式”和“详细模式”之间切换；简洁模式隐藏
  “最近流水”，详细模式保留原流水展示，选择写入账本设置并在重启后保留。
- 账单按钮区第一行显示今日、昨日，第二行只显示一个当前模式按钮；
  简洁模式下显示“简洁模式”并可切到详细，详细模式下显示“详细模式”并可
  切到简洁。`+0` 显示当前日切账期的完整账单，`/使用说明` 单独打开帮助。
- 今日账单和完整账单只统计当前日切账期，昨日账单统计上一账期。
  日切后当前账单的金额、笔数和明细归零；历史流水保留归档，不跨期累计。
- 主动查询今日、昨日或完整账单时，顶部入款和下发明细显示所选账期全部流水；
  简洁模式同样显示全部明细。新增记账后的自动回复仍只展示最近三笔明细。
  点击模式按钮切回简洁模式时收起为最近三笔；切到详细模式时展开全部明细。
  模式切换只改变明细展示数量，笔数和合计始终统计所选账期全部流水。
  超过单条消息长度的账单按现有 HTML 分段逻辑发送，模式切换同样保留全部分段。
- 入款、下发、撤销、清空和权限。
- 成功撤销后直接回复恢复后的当前账单，不额外显示“已撤销”流水说明。
- 账单回复直接发送，不引用原消息；长账单的续页同样不显示引用框。
- 裸负数（如 `-100`）是减分（入款冲减），与入款一起显示在“已入款”栏，绝不记入“已下发”；只有以“下发”开头的明确指令（如 `下发100`、`下发-200`）才创建下发流水，`/out`、`/payout`、`出款`、`下分`均不触发下发。
- 账单顶部的入款、下发明细优先显示回复对象或手工备注；详细模式在没有
  回复和备注时显示实际发送记账命令者的昵称，简洁模式不显示该默认昵称，
  只保留回复其他用户产生的归属或手工备注。最近流水保持简洁格式。
