...
- 多选群后确认操作，支持翻页、返回和取消。
- 关闭列表显示当前开启识别的群；开启列表只显示已关闭识别的群。
- 复用按群持久化的识别开关，关闭后新收到的图片不再进入识别，重启后保持。
- 只更改选中的群，并清除其待发送的上课/下课提醒，群内不发送操作提醒。
- 旧菜单和已确认菜单不能重复执行，广播和清理菜单的选择状态保持独立。
...
- 群级汇率、费率和日切。
- 群账务设置中的“当前账期”显示起始日期时间至下次日切日期时间；日切时间
  修改期间保留原账期起点，展示实际延续区间，不改变账期或流水。
- 新群默认每天北京时间 03:00 日切；群内主动设置后保留该群的自定义时间，
  重启不会覆盖已有设置。
- 群内修改日切后，已有当前账期延续到下一次新设定的日切时间，随后开启新账期。
  当前、昨日账期的衔接在重启后保留；历史流水和金额不回溯修改。
...
- 账单按钮区第一行显示今日、昨日，第二行只显示一个切换模式按钮；
  简洁模式下显示“↪️切换详细模式”，详细模式下显示“↪️切换简洁模式”。
  `+0` 显示当前日切账期的完整账单，`/使用说明` 单独打开帮助。
...
- 模式按钮按被点击的账单刷新，不因群级模式已经相同而跳过。
  新发送账单的续页编号持久化保存；切换时清理该账单旧续页，再发送需要的续页。
  超出可删除时限的续页尝试编辑为短占位，避免继续展示旧流水。
  旧版本未记录关联的历史续页无法自动定位，不按相邻消息编号推测删除。
- 入款、下发、撤销、清空和权限。
- 同群同一条记账消息只入账一次，重复投递静默跳过；不同消息的相同金额正常记账。
  消息编号和流水在同一数据库事务中写入；编号保留到重启、撤销和清空之后，
  避免旧消息重投重新入账。既有重复流水不自动删除或合并。
- 下发只回复账单，不再额外发送“已转，请您查收”；关闭记账时下发指令静默。
- 每天北京时间 09:00 检查该时刻最近已结束账期的归档流水；“未下发”余额为
  负数才向对应群发送日切提醒，余款显示绝对值并保留两位小数，正数和零不提醒。
  提醒中的时间使用该账期实际日切时间，支持群自定义日切；当前账期不混入。
  仅对已记录且启用记账的活跃群发送，不改动流水或余额，不引用消息。
  首次启用从下一个 09:00 开始，重启可补查当天 09:00 对应账期，不追发更早日期。
  群和账期的发送记录持久化；请求结果不确定时保留记录供排查，不自动重发，
  避免重复提醒。Telegram 明确要求限流等待时，按返回时长等待后重试。
