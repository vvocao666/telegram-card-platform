...
from trx_energy_bot.bot import handlers as trx_handlers
from trx_energy_bot.bot.keyboards import main_menu_keyboard
from trx_energy_bot.database import db as trx_db
from trx_energy_bot.database.models import ORDER_TYPE_ENERGY_RENTAL
from trx_energy_bot.services import order_service as trx_order_service
...
class BotFormattingTests(unittest.TestCase):
    ...
    def test_start_help_mentions_core_features(self):
        ...
        self.assertIn("TRX 能量租赁", help_text)
        self.assertIn("币价", help_text)
        self.assertEqual("https://t.me/kamibot?startgroup=true", keyboard.inline_keyboard[0][0].url)

    def test_trx_reply_keyboard_has_requested_menu_only(self):
        keyboard = main_menu_keyboard()
        labels = [[button.text for button in row] for row in keyboard.keyboard]

        self.assertEqual(
            labels,
            [["⚡ 能量租用", "✏️ 笔数套餐"], ["✅记账拉机器人进群"]],
        )

    def test_trx_energy_intro_matches_original_flow(self):
        message = trx_handlers._start_message()

        self.assertIn("波场能量租赁", message)
        self.assertIn("2.5TRX = 1 笔转账", message)
        self.assertIn("点击地址自动复制", message)

    def test_trx_old_and_new_reply_keyboard_labels_are_accepted(self):
        buy_re = re.compile(trx_handlers.BUY_ENERGY_TEXT_RE)
        smart_re = re.compile(trx_handlers.SMART_BUY_TEXT_RE)

        for text in ("⚡ 能量租赁", "⚡ 能量租用", "能量租赁", "能量租用"):
            self.assertRegex(text, buy_re)
        for text in ("✍️ 笔数套餐", "✏️ 笔数套餐", "笔数套餐"):
            self.assertRegex(text, smart_re)
    ...
    def test_help_includes_okx_price_command(self):
        ...
        self.assertIn("暂停/开启 - 暂停或开启记账", ledger_commands.HELP_TEXT)
    ...
    def test_ledger_commands_are_available_in_card_bot(self):
        with tempfile.TemporaryDirectory() as temp_dir:
            ...
            try:
                ...
                self.assertIn("未下发：<code>60U</code>", bill.text)
            finally:
                store.close()
    ...
    def test_ledger_rate_divides_amount(self):
        with tempfile.TemporaryDirectory() as temp_dir:
            ...
            try:
                ...
                self.assertIn("未下发：<code>100U</code>", bill.text)
                ...
                self.assertNotIn("未下发：<a", bill.text)
    ...
    def test_ledger_balance_uses_rmb_and_shows_usdt_side_by_side(self):
        with tempfile.TemporaryDirectory() as temp_dir:
            ...
            try:
                ...
                self.assertIn("未下发：<code>0U</code>", bill.text)
            finally:
                store.close()
    ...
    def test_trx_energy_module_creates_mock_order(self):
        with tempfile.TemporaryDirectory() as temp_dir:
            old_settings = trx_db.settings
            trx_db.settings = dataclass_replace(
                old_settings,
                database_url=f"sqlite:///{Path(temp_dir) / 'trx.sqlite3'}",
                default_packages=[{"name": "test_65k", "energy": 65000, "price": 2.5, "duration_hours": 1}],
            )
            try:
                trx_db.init_db()
                packages = trx_order_service.list_active_packages()
                order = trx_order_service.create_order_and_call_api(
                    user_telegram_id=12345,
                    user_nickname="@boss",
                    tron_address="TUpTiQHHPUEWrbeABxjjwu3WULbjxJ2ZCZ",
                    purchase_type=ORDER_TYPE_ENERGY_RENTAL,
                    transfer_count=None,
                    package=packages[0],
                )

                self.assertEqual(1, len(packages))
                self.assertTrue(order.order_no.startswith("TRX"))
                self.assertEqual("API_CREATED", order.order_status)
                self.assertEqual("test_65k", order.package_name)
            finally:
                trx_db.settings = old_settings
