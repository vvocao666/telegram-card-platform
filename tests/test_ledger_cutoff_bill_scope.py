...
def test_full_bill_resets_at_two_and_yesterday_keeps_previous_period(tmp_path, monkeypatch, view_mode):
    ...
    try:
        ...
        for text in ("+0", "今日账单"):
            bill = command(text)
            assert "总入款金额：73.00" in bill
            assert "已下发：10.00U" in bill
        ...
        assert "总入款金额：100.00" in yesterday
        assert "已下发：40.00U" in yesterday
